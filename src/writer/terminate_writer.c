...
int terminate_writer(pthread_t* writer, results_queue_t* results_queue) {
    ...
    if (!results_queue) {
        ...
        // Successfully forced termination, free memory allocated for writer
        fprintf(stderr, "Successfully terminated writer; successfully forced termination\n"); 
        free(writer); 
        
        // Successfully (forcefully) terminated writer
        return SUCCESS; 
    }
    ...
    if (reserved_index <= 0) {
        ...
        // Failed to terminate writer smoothly
        return ERROR; 
    }
