...
task_queue_t create_task_queue(size_t capacity) {
    ...
    queue.rle_tasks = (rle_task_t*)malloc(capacity * sizeof(rle_task_t));
    if (queue.rle_tasks == NULL) { // Check if memory allocation succeeded
    ...
    if (pthread_mutex_init(&queue.lock, NULL) != 0) {
        ...
        free(queue.rle_tasks); // Free allocated memory before exiting
        exit(1);
    };
...
void free_task_queue(task_queue_t* queue) {
    ...
    free(queue->rle_tasks);
    pthread_mutex_destroy(&queue->lock);
    pthread_cond_destroy(&queue->not_empty);
    pthread_cond_destroy(&queue->not_full);
}

rle_task_t get_task(task_queue_t *queue) {
    ...
    rle_task_t rle_task = queue->rle_tasks[queue->front];
...
int add_task(task_queue_t *queue, rle_task_t rle_task) {
    ...
    queue->rle_tasks[queue->rear] = rle_task;
